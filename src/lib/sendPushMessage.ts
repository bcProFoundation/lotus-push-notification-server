...
export const sendPushMessage = (id: string, subscription: Subscription, message: Message, options: any) => {
    logger.log('debug', 'Sending Push Message', { pushContent: message, subscription });
    return webpush.sendNotification(subscription.pushSubObj, JSON.stringify(message), options)
    .catch((error) => {
